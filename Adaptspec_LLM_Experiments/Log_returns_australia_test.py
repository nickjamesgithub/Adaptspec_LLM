...
for fname in sorted(files):
    ...
    print(f"\nRunning {ticker} ({len(log_returns)} obs)...")

    result  = adaptspec(log_returns, nexp_max=20, nbeta=10, niter=2000, nburn=1000, tmin=90, seed=42, verbose=True)
    summary = summarise(result)

    changepoints = [b[1] for b in summary["segment_boundaries"][:-1]]

    print(f"  modal nexp: {summary['modal_nexp']}  mean nexp: {summary['mean_nexp']:.2f}")
    print(f"  changepoints: {[pd.Timestamp(dates[cp]).date() for cp in changepoints]}")
    ...
    for i, cp in enumerate(changepoints):
        rows.append({
            ...
            "cp_index":  i + 1,
            "obs_index": cp,
            "date":      pd.Timestamp(dates[cp]).date(),
            "probability": summary["changepoint_proba"][cp],
        })
    ...
    ax1.set_title(f"{ticker}  |  modal nexp={summary['modal_nexp']}  mean={summary['mean_nexp']:.2f}")
...
out.to_csv(os.path.join(DATA_DIR, "adaptspec_changepoints.csv"), index=False)
print(f"\nDone. Saved to adaptspec_changepoints.csv")
